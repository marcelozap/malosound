(async()=>{
 ...
 count.textContent=entries.length===1?'1 research snapshot available.':entries.length+' research snapshots available.';
 select.disabled=entries.length<2;
 if(!entries.length){document.getElementById('report').hidden=true;document.querySelector('.map-layout').hidden=true;document.getElementById('catalysts').hidden=true;document.getElementById('cutoff').textContent='No research snapshots published yet. Historical trading days are in the journal archive.';return;}
 ...
 if(requested&&!entries.some(e=>e.id===requested)){document.getElementById('snapshot-request').textContent='No research snapshot for that date. Showing '+entries[0].date+'.';}
 ...
 }catch{document.getElementById('snapshot-count').textContent='Research snapshot availability could not be loaded.';document.getElementById('focus').textContent='Refresh to try again, or open the journal archive above.';document.getElementById('date').disabled=true;document.getElementById('report').hidden=true;}
})();
